...
def main(cfg: DictConfig):
    ...
    wandb.init(project=args.project_name,
           ...
           config=OmegaConf.to_container(args, resolve=False, throw_on_missing=False),
           mode='disabled' if args.project_name == 'disabled' else 'online')
    ...
    begin_learn = False
    episode_reward = 0
    training_rewards = []   # per-episode returns during training
    eval_history = []       # (learn_step, mean_eval_return) at each evaluation
    ...
    for epoch in count():
        ...
        for episode_step in range(EPISODE_STEPS):
            ...
            if learn_steps % args.env.eval_interval == 0:
                ...
                eval_history.append((learn_steps, float(returns)))
            ...
            if online_memory_replay.size() > INITIAL_MEMORY:
                ...
                if learn_steps == LEARN_STEPS:
                    ...
                    with open('training_curve.json', 'w') as _f:
                        _json.dump({
                            'episode_returns': training_rewards,
        ...
        training_rewards.append(float(episode_reward))
