...
class EnsembleCritic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=256, num_critics=5):
        super().__init__()
        self.num_critics = num_critics
        # Create N separate Q-networks
    ...
    def forward(self, state, action):
        ...
        # Returns shape: [batch_size, num_critics]
        return torch.cat([q_net(sa) for q_net in self.q_networks], dim=-1)
...
class CSILAgent:
    ...
    def __init__(
        ...
        device: str = "cpu",
        num_critics: int = 5,       # ensemble size for critic uncertainty estimation
    ):
        self.device = device
        self.action_dim = action_dim
        self.gamma = gamma
        ...
        self.critic = EnsembleCritic(state_dim, action_dim, num_critics=num_critics).to(self.device)
        self.critic_target = EnsembleCritic(state_dim, action_dim, num_critics=num_critics).to(self.device)
    ...
    def update(self, replay_buffer: ReplayBuffer, batch_size: int = 256) -> dict:
        ...
        with torch.no_grad():
            ...
            target_q_values = self.critic_target(next_state, next_action)
            
            # For the target, we use the pessimistic estimate (min over ensemble)
            target_q = target_q_values.min(dim=-1, keepdim=True)[0]
            target_q = reward + (1 - done) * 0.99 * (target_q - self.alpha * next_log_prob)

        # Update Critic
        current_q_values = self.critic(state, action)
        # MSE loss for all critics in the ensemble
        ...
        # Update Actor (This is where SOAR happens!)
        new_action, log_prob = self.actor(state)
        q_values_new_action = self.critic(state, new_action)
        
        if self.is_soar:
            # SOAR: Optimism in the face of uncertainty
            q_mean = q_values_new_action.mean(dim=-1, keepdim=True)
            q_std = q_values_new_action.std(dim=-1, keepdim=True)
            # Optimistic Q-value: Mean + Beta * StdDev
            q_eval = q_mean + beta * q_std
        else:
            # Standard SAC uses the pessimistic estimate
            q_eval = q_values_new_action.min(dim=-1, keepdim=True)[0]

        actor_loss = (self.alpha * log_prob - q_eval).mean()
